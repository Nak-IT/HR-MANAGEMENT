...
<h2 class="custom-font006 "><br><br>បញ្ជីបញ្ចូលព័ត៌មាន មន្ត្រីកិច្ចសន្យា​ឬមន្ត្រីជួល និង មិនមែនជាវេជ្ជសាស្ត្រ</h2>
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title custom-font007 text-centerModal text3D" id="exampleModalLabel">បញ្ចូលព័ត៌មាន មន្ត្រីកិច្ចសន្យា​ឬមន្ត្រីជួល និង មិនមែនជាវេជ្ជសាស្ត្រ</h5>
            ...
            <div class="modal-body">
                ...
                <form class="form-horizontal" id="hiredNotMedicalOfficerForm">
                    @csrf
                    <div class="row form-group">
                        <div class="col-sm-12">
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-sm-4">
                            <label for="EndDate">ថ្ងៃបញ្ចប់ការងារកិច្ចសន្យា​</label>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-sm-4">
                            <label for="SkillID">កម្រិតជំនាញ</label>
                            <select id="SkillID" name="SkillID" class="form-control">
                                <option value="">--ជ្រើសរើស--</option>
                                @if(isset($skills) && is_iterable($skills))
                ...
                <div class="details-info custom-font010">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong class="custom-font009">កម្រិតជំនាញ៖</strong> <span id="detailSkillName"></span></p>
...
var dropdownSkillID = new Choices(document.querySelector("#SkillID"));
...
function showData() {
    $.ajax({
        type: "GET",
        url: "{{ route('hired_not_medical_officers.getAllData') }}",
        dataType: 'json',
        success: function (alldata) {
            ...
            for (var i in alldata) {
                ...
                var DateInfo =
                    ...
                    "✍️" + "<span class='custom-font010'>ថ្ងៃ​ខែឆ្នាំបញ្ចប់កិច្ចសន្យា ៖ </span>" + "<br>" + "👉" + (formattedEndDate || "មិនទាន់ប្រាកដ");
                ...
                var DepartmentName =
                    "✍️" + "<span class='custom-font010'>កម្រិតជំនាញ ៖ </span>" + "<br>" + "👉" + (alldata[i].SkillName || "");
                ...
                data.push([
                    ...
                    "<button type='button' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#myModal' onClick='editData(" + alldata[i].HiredNotMedicalOfficerID + ")'><i class='fa fa-edit'></i></button> | " +
                    "<button type='button' class='btn btn-info' onClick='Details(" + alldata[i].HiredNotMedicalOfficerID + ")'><i class='fa fa-eye'></i></button> | " +
                    "<button type='button' class='btn btn-danger' onClick='deleteData(" + alldata[i].HiredNotMedicalOfficerID + ")'><i class='fa fa-trash'></i></button>"
                ]);
            }
...
function addNew() {
    ...
    dropdownSkillID.setChoiceByValue("");
...
function insertData() { 
    ...
    $.ajax({
        type: "POST",
        url: "{{ route('hired_not_medical_officers.create') }}",
        data: e,
        dataType: 'json',
        headers: {
        ...
        error: function (xhr) {
            if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                ...
                $.each(errors, function (key, value) {
                    if (key === 'EmployeeID') {
                        if (value[0].includes('បុគ្គលិកនេះមានរួចហើយក្នុងប្រព័ន្ធ។')) {
                            employeeIDUnique = true;
                        } else if (value[0].includes('សូមពិនិត្យឡើងវិញ: បុគ្គលិកនេះ មាននៅក្នុងបញ្ជីបុគ្គលិក/មន្ត្រីជួលនិងជាវេជ្ជសាស្ត្រា រួចហើយ។')) {
                            existsInMedicalOfficer = true;
                        }
                        else if (value[0].includes('សូមពិនិត្យឡើងវិញ: បុគ្គលិកនេះ មាននៅក្នុងបញ្ជីក្របខណ រួចហើយ។')) {
                ...
                if (employeeIDUnique) {
                    ...
                    Swal.fire({
                        title: 'កំហុស',
                        text: 'សូមពិនិត្យឡើងវិញ: បុគ្គលិកនេះ មាននៅក្នុងបញ្ជីក្របខណ រួចហើយ។',
                        icon: 'error'
                    });
                }
...
function editData(eId) {
    ...
    $.ajax({
        type: "GET",
        url: "{{ route('hired_not_medical_officers.getDataById', ['id' => ':id']) }}".replace(':id', eId),
        dataType: 'json',
        success: function (data) {
            if (data.error) {
            ...
            dropdownSkillID.setChoiceByValue(e.SkillID.toString());
        ...
        error: function (xhr, status, error) {
            ...
            if (xhr.status === 404) {
                ...
                Swal.fire({ title: "Error", text: "An error occurred while fetching data", icon: "error" });
            }
        }
    });
}
...
function updateData() {
    ...
    $.ajax({
        type: "PUT",
        url: "{{ route('hired_not_medical_officers.update', '') }}/" + update_id,
        data: e,
        dataType: 'json',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
function Details(id) {
    $.ajax({
        type: "GET",
        url: "{{ route('hired_not_medical_officers.getHiredNotMedicalOfficerDetails', ['id' => ':id']) }}".replace(':id', id),
        success: function (data) {
            if (!data) {
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: "{{ route('hired_not_medical_officers.delete', ':id') }}".replace(':id', id),
                data: {
                    "_token": "{{ csrf_token() }}"
                },
